...
# ... (all other functions remain exactly the same: build_dedicated_tls_ai_template, build_dedicated_n_tls_ai_template, etc.)
...
def build_v2rayng_template(remarks, outbound_nodes, pool_top_dns, pool_main_dns, clean_addresses=None, is_cloudflare=True):
    ...
    # === MODIFIED DNS SELECTION ACCORDING TO YOUR REQUEST ===
    ...
    # remote-dns-3,4,5: Any type EXCEPT raw IPv4/IPv6 from both lists
    non_ip_entries = []
    for p in pairs_top + pairs_main:
        srv = p["server"].strip().lower()
        if srv.startswith("https://") or srv.startswith("tcp:") or srv.startswith("quic:") or srv.startswith("udp:"):
            non_ip_entries.append(p)

    random.shuffle(non_ip_entries)
    remaining_slots = [2, 3, 4]
    for p in non_ip_entries:
        ...
        if ident not in seen_identifiers:
            ...
            chosen_providers[slot] = p

    # Fallback if any slot is empty
    for i in range(5):
        if chosen_providers[i] is None:
            for fb in fallback_providers:
                if fb["server"].startswith("https://") and get_identity_key(fb["server"]) not in seen_identifiers:
                    seen_identifiers.add(get_identity_key(fb["server"]))
                    chosen_providers[i] = fb
                    break
...
# (The rest of the script - main(), other build functions - remain 100% unchanged)
