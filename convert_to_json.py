...
def parse_standard_uri(url_str, protocol, tls_counter=[0], non_tls_counter=[0], clean_addresses=[], ip_counter=[0]):
    try:
        ...
        if protocol == "vless":
            ...
            # Extract method and password from standard userinfo (e.g. ss://method:password@host:port)
            if userinfo and ":" in userinfo:
                method, password = userinfo.split(":", 1)
            else:
                method, password = "chacha20-ietf-poly1305", userinfo or "password"
            outbound["settings"] = {
                "servers": [{
                    "address": target_address,
                    "level": 8,
...
def build_custom_protocol_template(remarks_title, protocol_nodes):
    """Builds a dedicated template structure strictly aligned with the requested format."""
    outbounds_list = []
    for idx, item in enumerate(protocol_nodes):
        node_copy = copy.deepcopy(item)
        node_copy["tag"] = f"prox-{idx + 1}"
        outbounds_list.append(node_copy)

    outbounds_list.extend([
    ...
    return {
        "remarks": remarks_title,
        "log": {
            "loglevel": "warning"
        },
        "dns": {
            "servers": [
                {"address": "https://dns.quad9.net/dns-query", "tag": "remote-dns-1"},
                {"address": "https://security.cloudflare-dns.com/dns-query", "tag": "remote-dns-2"},
                {"address": "udp://89.187.187.2:53", "tag": "remote-dns-3"},
                {"address": "https://8.8.8.8/dns-query", "tag": "remote-dns-4"},
                {"address": "tcp://45.11.45.11:853", "tag": "remote-dns-5"},
                {"address": "local", "domains": ["geosite:category-ir"], "expectIPs": ["geoip:ir"], "skipFallback": False},
                {"address": "78.157.42.100", "domains": ["geosite:category-ir"], "expectIPs": ["geoip:ir"], "skipFallback": False},
                {"address": "208.67.222.222", "domains": ["geosite:category-ir"], "expectIPs": ["geoip:ir"], "skipFallback": False}
            ],
            "queryStrategy": "UseIP",
            "tag": "dns"
        },
        "inbounds": [
            {
                ...
                "sniffing": {"destOverride": ["http", "tls"], "enabled": False, "routeOnly": False},
                "tag": "socks-in"
            },
            {
                "port": 10853,
        ...
        "routing": {
            ...
            "rules": [
                ...
                {"inboundTag": ["remote-dns-1", "remote-dns-2", "remote-dns-3", "remote-dns-4", "remote-dns-5"], "balancerTag": "all", "type": "field"},
            ...
            "balancers": [
                {"tag": "all", "selector": ["prox"], "strategy": {"type": "leastPing"}, "fallbackTag": "prox-1"}
            ]
        },
        "stats": {},
        "observatory": {
...
def build_v2rayng_template(remarks, outbound_nodes, pool_top_dns, pool_main_dns, ip_supplier, is_cloudflare=True):
    outbounds = []
    for idx, item in enumerate(outbound_nodes):
        node_copy = copy.deepcopy(item)
        if is_cloudflare:
            vnext_or_server = "vnext" if "vnext" in node_copy["settings"] else "servers"
            node_copy["settings"][vnext_or_server][0]["address"] = ip_supplier.get_next()
        outbounds.append(node_copy)
    ...
    paired_top = parse_dns_source(pool_top_dns)
    paired_main = parse_dns_source(pool_main_dns)
    
    dns_servers = []
    routing_rules = []
    
    for idx, item in enumerate(paired_top):
        tag = f"remote-dns-{idx + 1}"
        dns_servers.append({"address": item["server"], "tag": tag})
        if item["ip"]:
            routing_rules.append({"ip": [item["ip"]], "outboundTag": "direct", "port": 53, "type": "field"})
            
    for item in paired_main:
        dns_servers.append({
            "address": item["server"],
    ...
    routing_rules.extend([
        {"inboundTag": ["dns-in"], "outboundTag": "dns-out", "type": "field"},
        {"port": 53, "outboundTag": "dns-out", "type": "field"},
        {"domain": ["geosite:category-ir"], "outboundTag": "direct", "type": "field"},
        {"ip": ["geoip:ir"], "outboundTag": "direct", "type": "field"},
        {"network": "tcp,udp", "balancerTag": "all", "type": "field"}
    ])
    ...
    return {
        "remarks": remarks,
        "log": {"loglevel": "warning"},
        "dns": {"servers": dns_servers, "queryStrategy": "UseIP", "tag": "dns-module"},
        "inbounds": [
            {"port": 10808, "protocol": "socks", "settings": {"auth": "noauth", "udp": True, "userLevel": 8}, "sniffing": {"destOverride": ["http", "tls"], "enabled": True, "routeOnly": False}, "tag": "socks-in"},
...
def process_subscription(input_file, output_file):
    print("🚀 Execution Started...")
    
    clean_ips = fetch_clean_addresses(CLEAN_IPS_URL)
    ip_supplier = CleanIPSupplier(clean_ips)
    ...
    print(f"📖 Loaded {len(lines)} raw links from input subscription file.")
    
    groups = {
        "vmess_tls": [], "vmess_ntls": [],
        "vless_tls": [], "vless_ntls": [],
        "trojan_tls": [], "trojan_ntls": [],
        "other_protocols": {} # Changed from list to dict for separate processing
    }
    ...
    for line in lines:
        ...
        if line.startswith("vmess://"):
            node, is_tls = parse_vmess(line, tls_counter, non_tls_counter)
            if node:
                groups["vmess_tls" if is_tls else "vmess_ntls"].append(node)
        else:
            detected_protocol = None
            for p in ["vless", "trojan", "ss", "shadowsocks"]:
                if line.startswith(f"{p}://"):
                    detected_protocol = "shadowsocks" if p == "ss" else p
                    break
                    
            if detected_protocol:
                if detected_protocol in ["vless", "trojan"]:
                    node, is_tls = parse_standard_uri(line, detected_protocol, tls_counter, non_tls_counter, clean_ips, ip_counter)
                    if node:
                        # Determine if destination is Cloudflare IP/Domain
                        orig_addr = node.get("_original_address", "")
                        is_cf = False
                        if orig_addr:
                            if "cloudfront" in orig_addr or "workers.dev" in orig_addr or "pages.dev" in orig_addr:
                                is_cf = True
                            else:
                                try:
                        ...
                        if is_cf:
                            if detected_protocol == "vless":
                                groups["vless_tls" if is_tls else "vless_ntls"].append(node)
                            elif detected_protocol == "trojan":
                                groups["trojan_tls" if is_tls else "trojan_ntls"].append(node)
                        else:
                            if detected_protocol not in groups["other_protocols"]:
                                groups["other_protocols"][detected_protocol] = []
                            groups["other_protocols"][detected_protocol].append(node)
                else:
                    # Directly route shadowsocks or fallback protocols to separate protocol buckets
                    node, is_tls = parse_standard_uri(line, detected_protocol, tls_counter, non_tls_counter)
                    if node:
                        if detected_protocol not in groups["other_protocols"]:
                            groups["other_protocols"][detected_protocol] = []
                        groups["other_protocols"][detected_protocol].append(node)
    ...
    # 1. 🤖 1 VLESS - TLS AI 🔥
    if groups["vless_tls"]:
        final_output.append(build_dedicated_tls_ai_template(f"🤖 1 VLESS - TLS AI 🔥 {tehran_time}", groups["vless_tls"], ip_supplier))
        
    # 2. 🍀 2 VMESS - TLS 🔥
    if groups["vmess_tls"]:
        for idx, item in enumerate(groups["vmess_tls"]):
            item["tag"] = f"prox-{idx + 1}"
        final_output.append(build_v2rayng_template("🍀 2 VMESS - TLS 🔥", groups["vmess_tls"], pool_top_dns, pool_main_dns, ip_supplier, is_cloudflare=True))
        
    # 3. 🍄 3 TROJAN - TLS 🔥
    if groups["trojan_tls"]:
        for idx, item in enumerate(groups["trojan_tls"]):
            item["tag"] = f"prox-{idx + 1}"
        final_output.append(build_v2rayng_template("🍄 3 TROJAN - TLS 🔥", groups["trojan_tls"], pool_top_dns, pool_main_dns, ip_supplier, is_cloudflare=True))
        
    # 4. ☘️ 4 VLESS - Non-TLS AI 🤖
    if groups["vless_ntls"]:
        final_output.append(build_dedicated_n_tls_ai_template(groups["vless_ntls"], ip_supplier))
        
    # 5. 🍁 5 VMESS - Non-TLS 🔥
    if groups["vmess_ntls"]:
        for idx, item in enumerate(groups["vmess_ntls"]):
            item["tag"] = f"prox-{idx + 1}"
        final_output.append(build_v2rayng_template("🍁 5 VMESS - Non-TLS 🔥", groups["vmess_ntls"], pool_top_dns, pool_main_dns, ip_supplier, is_cloudflare=True))
        
    # 6. 💐 6 TROJAN - Non-TLS 🔥
    if groups["trojan_ntls"]:
        for idx, item in enumerate(groups["trojan_ntls"]):
            item["tag"] = f"prox-{idx + 1}"
        final_output.append(build_v2rayng_template("💐 6 TROJAN - Non-TLS 🔥", groups["trojan_ntls"], pool_top_dns, pool_main_dns, ip_supplier, is_cloudflare=True))
    ...
    # Process isolated specific Protocol Groups separately
    if groups["other_protocols"]:
        for proto, nodes in groups["other_protocols"].items():
            remarks_name = f"🌲 7 {proto.upper()} - LB 🔥"
            final_output.append(build_custom_protocol_template(remarks_name, nodes))
    ...
    print(f"🎉 Successfully completed parsing processes! Structured items written into: {output_file}")

if __name__ == "__main__":
    process_subscription("sub.txt", "output.json")
